...
class Toast: NSObject {
    ...
    static func makeToast(_ message: String = "") {
        guard let superview = UIApplication.shared.connectedScenes.map({ $0 as? UIWindowScene }).compactMap({ $0 }).first?.windows.first else { return }
        superview.makeToast(message.isEmpty ? "Please check your internet connection and try again.!!" : message, duration: 1.5, position: .bottom)
    }
    ...
    /*static func show(_ message: String = "") {
        DispatchQueue.main.async {
            ...
            toastLabel.text = message.isEmpty ? "Please check your internet connection and try again.!!" : message
