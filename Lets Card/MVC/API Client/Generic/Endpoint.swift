...
extension Endpoint {
    ...
    func getRequest<T: Encodable>(_ query: T) -> URLRequest? {
        ...
        let headers = [HTTPHeader.contentType("application/json"), HTTPHeader.authorization(APIClient.shared.getAuthenticationWithoutBearer())]
...
//    }
    ...
    func postRequest<T: Encodable>(_ query: String, parameters: T) -> URLRequest? {
        ...
        let headers = [HTTPHeader.contentType("application/x-www-form-urlencoded"), HTTPHeader.authorization(APIClient.shared.getAuthenticationWithoutBearer())]

        do {
//            var json = try JSONEncoder().encode(parameters)
